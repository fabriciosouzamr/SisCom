...
/*            sXML = Arquivo.DiretorioSistema_AdicionarPath(sXML);
                                                               ...
                                                               string sJustificativa = "")
        {
            ...
            else
            {
                oNFe_Servico_RetornoRecepcaoEvento = oNFe_Servico.RecepcaoEventoCancelamento(1, 1, notaFiscalSaidaViewModel.Protocolo, 
                                                                                                   notaFiscalSaidaViewModel.CodigoChaveAcesso, 
                                                                                                   sJustificativa, 
                                                                    ...
                                                                    ref string sCD_NFCe_Token_CSC)
        {
            ...
            try
            {
                ...
                oNFe.infNFe.ide.dhSaiEnt = Validacao.Data_AdicionarHora(notaFiscalSaidaViewModel.DataSaida, notaFiscalSaidaViewModel.HoraEmissao);
                ...
                if (!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.InscricaoEstadual))
                    oNFe.infNFe.emit.IE = TratarString(notaFiscalSaidaViewModel.Empresa.InscricaoEstadual);
                if (!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.InscricaoMunicipal))
                {
                    oNFe.infNFe.emit.IM = TratarString(Funcao.NuloParaString(notaFiscalSaidaViewModel.Empresa.InscricaoMunicipal));
                    oNFe.infNFe.emit.CNAE = TratarString(Funcao.NuloParaString(notaFiscalSaidaViewModel.Empresa.InscricaoMunicipal));
                }
                ...
                {
                    if (!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.Endereco.End_Logradouro)) 
                    { oNFe.infNFe.emit.enderEmit.xLgr = TratarString(Funcao.NuloParaString(notaFiscalSaidaViewModel.Empresa.Endereco.End_Logradouro)); }
                    if (!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.Endereco.End_Numero)) 
                    { oNFe.infNFe.emit.enderEmit.nro = TratarString(Funcao.NuloParaString(notaFiscalSaidaViewModel.Empresa.Endereco.End_Numero).Trim()); }
                    if (!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.Endereco.End_Complemento)) 
                    { oNFe.infNFe.emit.enderEmit.xCpl = TratarString(Funcao.NuloParaString(notaFiscalSaidaViewModel.Empresa.Endereco.End_Complemento).Trim()); }
                    if (!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.Endereco.End_Bairro)) 
                    { oNFe.infNFe.emit.enderEmit.xBairro = TratarString(Funcao.NuloParaString(notaFiscalSaidaViewModel.Empresa.Endereco.End_Bairro).Trim()); }
                    if (notaFiscalSaidaViewModel.Cliente.Endereco.End_Cidade != null)
                    {
                        if (!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.Endereco.End_Cidade.CodigoIBGE)) 
                ...
                if (!bClienteNaoInformado)
                {
                    ...
                    if (!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Cliente_EMail))
                        oNFe.infNFe.dest.email = TratarString(notaFiscalSaidaViewModel.Cliente_EMail);
                    ...
                    if ((!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Cliente.InscricaoEstadual)) && (notaFiscalSaidaViewModel.Cliente.InscricaoEstadual.Trim().ToUpper() != "ISENTO"))
                        oNFe.infNFe.dest.IE = TratarString(notaFiscalSaidaViewModel.Cliente.InscricaoEstadual);
                }
                ...
                if (oNFe.infNFe.transp != null)
                {
                    ...
                    {
                        vol.esp = TratarString(notaFiscalSaidaViewModel.VolumeTransportados_Especie);
                    }
                ...
                foreach (NotaFiscalSaidaMercadoriaViewModel ntFSMercadoria in notaFiscalSaidaMercadoriaViewModels)
                {
                    ...
                    if (String.IsNullOrEmpty(ntFSMercadoria.Mercadoria.CodigoBarra))
                        ...
                        oNFE_Det.prod.cEAN = TratarString(ntFSMercadoria.Mercadoria.CodigoBarra);
                    ...
                    if (ntFSMercadoria.Mercadoria.Fiscal_NFS_TributosTotal > 0)
                        totaltributos = String.Format("{0} ({1}%)", 
                                                      Valor.Percentagem(oNFE_Det.prod.vProd, ntFSMercadoria.Mercadoria.Fiscal_NFS_TributosTotal).ToString("C"), 
                                                      ntFSMercadoria.Mercadoria.Fiscal_NFS_TributosTotal);
                ...
                if (oNFe.infNFe.ide.mod == ModeloDocumento.NFe)
                {
                    ...
                    {
                        var dups = new List<NFe.Classes.Informacoes.Cobranca.dup>();
                        var iDup = 0;

                        foreach (NotaFiscalSaidaPagamentoViewModel notaFiscalSaidaPagamentoViewModel in notaFiscalSaidaPagamentoViewModels)
                        {
                            if (FormaPagamentoDBParaZeus(notaFiscalSaidaPagamentoViewModel.FormaPagamento) == NFe.Classes.Informacoes.Pagamento.FormaPagamento.fpDuplicataMercantil)
                            {
                                iDup = iDup + 1;

                                var dup = new NFe.Classes.Informacoes.Cobranca.dup();
                                dup.dVenc = notaFiscalSaidaPagamentoViewModel.DataVecimento;
                                dup.vDup = notaFiscalSaidaPagamentoViewModel.Valor;
                                dup.nDup = iDup.ToString("000");
                                dups.Add(dup);
                            }
                        }
                ...
                if ((!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.NFE_InfComple_TributosTotal)) && (!String.IsNullOrEmpty(totaltributos)))
                {
                    sDS_INFORMACOES_ADICIONAIS = notaFiscalSaidaViewModel.Empresa.NFE_InfComple_TributosTotal.Replace("[Trib]", totaltributos) +
                                                 (String.IsNullOrWhiteSpace(sDS_INFORMACOES_ADICIONAIS) ? "" : ". " + sDS_INFORMACOES_ADICIONAIS);
                }

                if ((!String.IsNullOrEmpty(notaFiscalSaidaViewModel.Empresa.NFE_InfComple_CreditoSimplesNacional)) && (notaFiscalSaidaViewModel.Empresa.CreditoSimplesNacional > 0))
                {
                    sDS_INFORMACOES_ADICIONAIS = notaFiscalSaidaViewModel.Empresa.NFE_InfComple_CreditoSimplesNacional.Replace("[Trib]", notaFiscalSaidaViewModel.Empresa.CreditoSimplesNacional.ToString()) +
                                                 (String.IsNullOrWhiteSpace(sDS_INFORMACOES_ADICIONAIS) ? "" : ". " + sDS_INFORMACOES_ADICIONAIS);
                }
                ...
                */
                ...
                if ((notaFiscalSaidaViewModel.Status != NF_Status.Transmitida) && (String.IsNullOrWhiteSpace(notaFiscalSaidaViewModel.CodigoChaveAcesso)))
                                                                                                    ...
                                                                                                    ref ManifestoEletronicoDocumentoViewModel manifestoEletronicoDocumento)
        {
            ...
            if (!String.IsNullOrEmpty(nrrecibo))
            {
                try
                {
                    ...
                    {
                        manifestoEletronicoDocumento.DataRetornoSefaz = DateTime.Now;
                        manifestoEletronicoDocumento.RetornoSefaz = retorno.XMotivo;
        ...
        public static ManifestoEletronicoDocumentoViewModel Fiscal_ManifestoEletronicoDocumento_Cancelar(ManifestoEletronicoDocumentoViewModel manifestoEletronicoDocumento, string justificativa)
        {
            ...
            MDFeEletronico mdfe;
            var caminhoXml = Path.Combine(Declaracoes.externos_Path_NuvemFiscal_MDFe, manifestoEletronicoDocumento.Autorizacao_ChaveAutenticacao + "-mdfe.xml");
                                                             ...
                                                             string xml = "")
        {
            ...
            try
            {
                ...
                if ((!String.IsNullOrEmpty(xml)) && !SomenteGerarXML)
                {
                    ...
                    retEnviNFe retorno = new retEnviNFe();

                    try
                    {
                        ponto = $"Zeus.Protocolar ({xml})";
                        retorno = Zeus.Protocolar(xml, recibo, notaFiscalSaidaViewModel.CodigoChaveAcesso, notaFiscalSaidaViewModel.Status == NF_Status.Finalizada);
                    }
                    catch (Exception ex1)
                    {
                        CaixaMensagem.Informacao($"NF-e Transmitir - {ponto}", ex1);
                    }
                    ponto = "Protocolado";
        ...
        public async static void Fiscal_Visualizar(Guid id, MeuDbContext meuDbContext, INotifier notifier)
        {
            try
            {
                ...
                using (NotaFiscalSaidaPagamentoController notaFiscalSaidaPagamentoController = new NotaFiscalSaidaPagamentoController(meuDbContext, notifier))
                {
        ...
        public static async Task MDFe_Transmitir(Guid id, MeuDbContext meuDbContext, INotifier notifier, ViewModels.EmpresaViewModel empresa)
        {
            ...
            if (manifestoEletronicoDocumento.Status == MDFe_Status.Autorizado)
            {
                CaixaMensagem.Informacao("MDF-e já autorizado");
                return;
            }
